...
class PATCHParams: Requestoble {
  ...
  func type(data: Any?, completion: @escaping ClousureRequest) {
    ...
    let url    = urls.create(type: .params(.users, .userID, .params))?.URL
    let header = headers.create(type: .autorization_appJson)
    let body   = urlBody.createEncoding(type: encode)
    //Request
    self.jsonDecode.decode(jsonType: DECParams.self, url: url, header: header, body: body, httpMethod: .patch) { (decodeResult) in
      //Responce
      switch decodeResult {
        //Error
        ...
        case .json(let object):
          if let params = object as?  DECParams{
