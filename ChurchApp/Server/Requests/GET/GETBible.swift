...
class GETBible: Requestoble {
  ...
  private var urlBody    = URLBody()
    ...
    let url    = self.urls.create(type: .getBible(.bible, .covenants))?.URL
    let header = self.headers.create(type: .autorization_appJson)
    
		//Request
    self.jsonDecode.decode(jsonType: [DECBible].self, url: url, header: header, httpMethod: .get) { (decodeResult) in
			//Responce
			switch decodeResult {
				//Error
          ...
          if let bible = object as? [DECBible] {
					completion(.object(bible))
        }
			}
		}
	}
